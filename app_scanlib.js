...
const scrapePage = (htmlBody) => {
    ...
    const prepareDetails = (detailsElem) => {
        return detailsElem.querySelectorAll("p");
    };
    ...
    const container = htmlBody.querySelector(".thecontent") || htmlBody.querySelector(".entry-content");
    if (!container) {
        logLineAsync(logFN, "Не найден элемент .thecontent или .entry-content на странице ");
        return null;
    }

    const categoryItemsContainer = htmlBody.querySelector("span.thecategory") || htmlBody.querySelector("span.cat-links");

    const categoryItems = categoryItemsContainer.children;
    ...
    let imgTag = htmlBody.querySelector(".thecontent > img") || htmlBody.querySelector(".entry-content > img");
    ...
    const descrDetails = getDescendantListByTag(container, "details") || getDescendantListByTag(container, ".spoiler-wrap");
    ...
    while (counter < childNodes.length) {
        ...
        if (curr === underPicDataTag) {
            ...
            let tocNodes = prepareDetails(curr);
            let result = "";
            Array.from(tocNodes).forEach(node => {
                result += node.innerHTML;
            })
            let tocList = prepareList(result);
            description.push({ol: tocList});
        }
...
const readLocal = pathString => {
    return new Promise(async (resolve, reject) => {
        try {
            // проверка на наличие файла
            await isExists(pathString);
            fs.readFile(pathString, "utf8", (err, data) => {
                if (err) {
                    ...
                    let pageData = scrapePage(htmlBody);
                    if (!pageData) {
                        let errLine = "ОШИБКА! Не удалось сохранить в json данные со страницы " +
                            pathString;
                        logLineAsync(
                            logFN, errLine
                        );
                    ...
                    let cursor = pathString.lastIndexOf("\\");
                    let savePath = path.join(__dirname, "data", pathString.slice(cursor + 1).split(".html")[0].concat(".json"));
                    fs.writeFile(savePath, JSON.stringify(pageData), (err) => {
                        if (err) {
                            logLineAsync(
                                logFN,
